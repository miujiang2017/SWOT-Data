function [vali_estmed, ...
          ...
          validation4(Qest_med, sg_path, nR,use_svs) %#ok<INUSD>
% VALIDATION4: 从当前 path 最早有原始 SWOT Q 产品的日期开始做验证
%
% 起始日期按 path 统一判断：
%   min(first geoBAM raw, first SIC4DVar raw, first MOMMA raw, first MetroMan raw)
% across all reaches in this path. 同一个 path 内所有 reach 共用这个起始日期。
%
...
%% ===== 1. 当前 path validation 的统一起始日期 =====
start_day_idx = local_first_raw_path_day_idx(sg_path, nR);
...
%% ===== 内部小函数：找当前 path 最早有原始产品的 day index =====
function start_day_idx = local_first_raw_path_day_idx(sg_path, nR)

path_start_day_idx = nan;
dayidx_fields = { ...
    % 'day_index_geoBAM', ...
    'day_index_SIC4DVar', ...
    'day_index_MOMMA', ...
    ...
    for j = 1:nR
        if numel(day_idx) >= j && ~isempty(day_idx{j})
            ...
            if ~isempty(valid_days)
                ...
                if isnan(path_start_day_idx) || candidate < path_start_day_idx
                    path_start_day_idx = candidate;
                end
            end
        end
    end
...
start_day_idx = repmat(path_start_day_idx, nR, 1);
