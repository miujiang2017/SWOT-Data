function [group_vali_out, group_median, all_group_vali] = validation_flow_group_all(data_KF_out, Q_results, use_svs)
% validation_flow_group_all
%
% INPUT:
%   data_KF_out
%   Q_results
...
path_vali.Qest_med = calc_Qestmed_vali(Qest_med, Q_gauge, nR, groups);

path_vali.SIC4DVar = calc_product_vali_safe( ...
    sg_path, 'Q_SIC4DVar', 'day_index_SIC4DVar', Q_gauge, nR, groups, 2);

path_vali.MOMMA = calc_product_vali_safe( ...
    sg_path, 'Q_MOMMA', 'day_index_MOMMA', Q_gauge, nR, groups, 2);

path_vali.geoBAM = calc_product_vali_safe( ...
    sg_path, 'Q_geoBAM', 'day_index_geoBAM', Q_gauge, nR, groups, 2);

path_vali.SADS = calc_product_vali_safe( ...
    sg_path, 'Q_SADS', 'day_index_SADS', Q_gauge, nR, groups, 2);

path_vali.MetroMan = calc_product_vali_safe( ...
    sg_path, 'Q_MetroMan', 'day_index_MetroMan', Q_gauge, nR, groups, 2);

path_vali.SIC4DVar_interp = calc_product_vali_safe( ...
    sg_path, 'Q_SIC4DVar', 'day_index_SIC4DVar', Q_gauge, nR, groups, 3);

path_vali.MOMMA_interp = calc_product_vali_safe( ...
    sg_path, 'Q_MOMMA', 'day_index_MOMMA', Q_gauge, nR, groups, 3);

path_vali.geoBAM_interp = calc_product_vali_safe( ...
    sg_path, 'Q_geoBAM', 'day_index_geoBAM', Q_gauge, nR, groups, 3);

path_vali.SADS_interp = calc_product_vali_safe( ...
    sg_path, 'Q_SADS', 'day_index_SADS', Q_gauge, nR, groups, 3);

path_vali.MetroMan_interp = calc_product_vali_safe( ...
    sg_path, 'Q_MetroMan', 'day_index_MetroMan', Q_gauge, nR, groups, 3);
...
%% ========================================================================
function vali = calc_Qestmed_vali(Qest_med, Q_gauge, nR, groups)
...
for icase = 1:nCase
    ...
    for j = 1:nR
        ...
        if j > numel(Q_gauge) || isempty(Q_gauge{j})
            continue
        end
...
%% ========================================================================
function vali = calc_product_vali_safe(sg_path, q_field, day_field, Q_gauge, nR, groups, mode_flag)
...
for j = 1:nR
    ...
    if isempty(idx_validQ)
        continue
    end

    Q_prod_comp = Q_prod(:,2:end);

    group_idx = get_group_idx(Q_true_all, idx_validQ);

    if mode_flag == 2

        idx_prod = day_idx{j} - 1;
        idx_available = intersect(idx_validQ, idx_prod);
    ...
    elseif mode_flag == 3
        ...
        Q_interp_at_gauge = interp1(idx_prod, Q_tmp, idx_validQ, 'linear', 'extrap');
        ...
        for ig = 1:numel(groups)
            ...
            [idx_g, loc] = intersect(idx_g_global, idx_validQ);
            ...
            Q_true_raw = Q_true_all(idx_g);
            Q_comp_raw = Q_interp_at_gauge(loc);
