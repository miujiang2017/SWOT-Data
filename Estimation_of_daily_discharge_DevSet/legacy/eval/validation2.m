function [vali_estmed, ...
          ...
          validation2(Qest_med, sg_path, nR,use_svs) %#ok<INUSD>
...
% 其中：
%   - 对 vali_estmed.error:
%         error{i}{j} = 第 i 个 Qest_med 结果中，第 j 个 reach 的 abs(Qest-Qgauge)
%   - 对其他产品:
%         error{j}    = 第 j 个 reach 的 abs(Qprod-Qgauge)
%
% 空产品时四个指标为 NaN，error 为空 cell。
...
[corr_med, ~, rRMSE_med, NSE_med, rB_med, error_med] = ...
    vali_calc(Qest_med, sg_path.SVS_Q{1,1}, nR, [], 1);
else
[corr_med, ~, rRMSE_med, NSE_med, rB_med, error_med] = ...
...
function vali_str = local_safe_vali(sg_path, q_field_name, dayidx_field_name, ...
    nR, mode_flag, make_empty_vali,use_svs)
...
if use_svs
    [corr_v, ~, rRMSE_v, NSE_v, rB_v, error_v] = ...
        vali_calc(Q_cell, sg_path.SVS_Q{1,1}, nR, day_idx, mode_flag);
else
    [corr_v, ~, rRMSE_v, NSE_v, rB_v, error_v] = ...
...
function [corr_comp_true, RMSE_comp_true, rRMSE_comp_true, NSE_comp_true, rB_comp_true, error_comp_true] = ...
    vali_calc(Q_est, Q, nR, reserve, opt)
...
elseif opt == 3
    ...
    for j = 1:nR
        if ~isempty(reserve{j}) && ~isempty(Q{j})
            ...
            if length(idx_valid) > 1
                Q_est_comp = Q_est(:,2:end);
                Q_tmp = cell2mat(Q_est_comp(j,idx_valid))';

                % 插值到 gauge 有值的时刻
                Q_comp_raw = interp1(idx_valid, Q_tmp, idx_validQ, 'linear', 'extrap');
                Q_true_raw = Q_tmp_true(idx_validQ);
